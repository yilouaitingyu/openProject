...
require(['Util', 'list', 'indexLoad', 'date', 'dialog', 'simpleTree','selectTree', 'ajax','validator'],
		function(Util, List, IndexLoad, MyDate, Dialog, SimpleTree,SelectTree, ajax,Validator){
			...
			var eventInit = function() {
				...
				creartSelect("CCT_USERSATISFY_HLWKF_FIR", "满意度","feedbackSatisfied");
				/**事件部分*/
				...
				/**请求类树触发事件*/
				crossAPI.on("acceptingDepartment",function(param){
					$('#acceptStaffdept').val(param.nodename);
					$("#acceptStaffdept").attr('val',param.id);
				})
				/**手里部门树触发事件*/
				crossAPI.on("serviceCategory",function(param){
	                ...
	                button:[
							{
							    ...
							    callback: function () {
				    			    ...
				    			    Util.ajax.postJson('/ngwf_he/front/sh/serviceReq!execute?uid=dialogRecord002',param,function(result){
				    			    	console.log(result);
				    			    });
							    }
			...
			var selectTreeBox = function(){
            	...
            	_index.showDialog({
					  ...
					  param:{tabName:"请求查询"},    //要传递的参数，可以是json对象
					  modal:false,
					  width: 800,
			          height: 520, 
					});
...
//            }
            ...
            var selectDeptTree = function(){
            	...
            	_index.showDialog({
					  ...
					  param:{tabName:"受理部门"},    //要传递的参数，可以是json对象
					  modal:false,
					  left:200,
					  width: 520,
			          height: 350,
...
//            }
			...
			var searchForm = function() {
				...
				if(startTime==''){
					crossAPI.tips("开始时间不可为空！",3000);
					return;
				}
				if(endTime==''){
					crossAPI.tips("结束时间不可为空！",3000);
					return;
				}
				if(!validator.form()){
					return;
				...
				if((dayInterval(end)-dayInterval(start))>10){
					crossAPI.tips("时间间隔不能超过10天!",3000);
					return;
				}
				if(end<start){  
				    crossAPI.tips("开始日期不能大于结束日期！",3000); 
				    return;
				}
				if(srtypeId=='' && id=='' && subsNumber=='' && acceptStaffno=='' && subsCity=='' && callerNo=='' &&  acceptStaffdept==''){
					crossAPI.tips("带*的条件必须任选一个!",3000);
					return;
				}
				listSearch(data);
			}
			...
			var staffnoDetail = function(e) {
				e.stopPropagation()
				var evt =e|| event;
		    	var bbb =$(this).parent('td').offset().left-192;
			...
			var  reqDetail =function (e){
				...
				Util.ajax.postJson('/ngwf_he/front/sh/serviceReqDetail!execute?uid=selectNumber',{"numberId":serviceId},function(data){
					if(data.beans.length>0){
						...
						crossAPI.tips("抱歉,该服务请求已经不存在！",3000);
					}
				})
			}
...

								{
									...
									render : function(item, val) {
					                	...
					                	$.each(_operationStatus,function(index,bean){
					                		if(bean.value==item.operationStatus){
					                			val=bean.name;
					                		}
					                	});
					                	return val;
									...
									render : function(item) {
					                	...
					                	$.each(_feedbackSatisfied,function(index,bean){
					                		if(bean.value==item.feedbackSatisfied){
					                			val=bean.name;
					                		}
					                	});
					                	return val;
			...
			IndexLoad(function(IndexModule, options) {
				...
				$('#acceptStaffno').val(staffId);
				setTimeout(function(){
					listSearch({startTime:startTime,endTime:endTime,acceptStaffNo:staffId});
				},10);
				eventInit();
			})
	})
