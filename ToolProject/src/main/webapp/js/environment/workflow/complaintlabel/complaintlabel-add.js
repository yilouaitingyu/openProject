require(
		...
		function(MyDate, List,Select,Dialog, $,Util,IndexLoad) {		   
             ...
             $("#save").click(function(){
            	 ...
            	 Util.ajax.getJson('/ngwf_he/front/sh/workflow!execute?uid=saveComplaintlabel', { labelName:labelName,rule:rule }, 
            			 function(result, isOk){
            		     if(result.bean.state=='0'){
            		    	 message("添加成功");
            		    	 isSave = true;
            		    	 param.complaintId = result.bean.complaintId;
                              ...
                              $("#importBtn").click(function () {
                        	   ...
                        	   //导入excel 的事件
              				  ...
              				  if(data.result.bean.state=="0"){
              					  message("导入数据成功")
              					  list.search({complaintId : param.complaintId});
              				  }else{
              					  message("导入数据失败") 
              				  }   
              			 },
