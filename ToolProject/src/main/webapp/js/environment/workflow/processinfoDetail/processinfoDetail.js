			 			...
			 			Util.ajax.postJson(
		            	 		...
		            	 		for(var item in beans){
		            	 			...
		            	 			if(typeof(beans[item].action)!='undefined' && beans[item].action!=""){
			            	 			...
			            	 			//判断是否为外省派发,添加样式
			            	 			if (processinfo.ORDER_TYPE!='1') {
			            	 				if(beans[item].actionId=="201704182000598120335796432122"||beans[item].actionId=="201704194567891333333222211111"){
			            	 					$('#'+beans[item].lineid).removeClass('show').addClass('hide');
				            	 			}
											}
			            	 			
			            	 			
			            	 			//为工单添加操作按钮事件
			            	 			$("#"+beans[item].lineid).off("click").on('click',function(){
...
//			            	 		  		                }
		...
		var eventInit=function(){
			 ...
			 $("#saveAgain").on('click',saveAgain);//复核阶段修改工单信息保存工单异动 
		};
		var saveAgain = function(){
			var formJson = $("#aor_form").serializeObject();
			  formJson.srvReqstId = processinfo.srvReqstId;
			  formJson.wrkfmSwftno = processinfo.wrkfmShowSwftno;
			  formJson.modfStaffNum = _options.loginStaffId;
			  var url="";
			  if(processinfo.trash_flag){
				  url='/ngwf_he/front/sh/workflow!execute?uid=detailDataUpdate'
			  }else{
				  url='/ngwf_he/front/sh/workflow!execute?uid=detailDataUpdate'
			  }
			  formJson.wrkfmId = processinfo.wrkfmId;
			  Util.ajax.postJson('/ngwf_he/front/sh/workflow!execute?uid=detailDataUpdate', formJson, function(json, status) {
				  if(json.returnCode=="0"){
					  crossAPI.tips("修改保存成功！",3000);
				  }
			  })
		};
...
//				console.log("aaa")
		...
		var tabContainerInit = function(){
			...
			Util.ajax.postJson('/ngwf_he/front/sh/workflow!execute?uid=queryAttachment',params,function(result){
				if(typeof(result.bean.total)!='undefined'){
    				attachmentCount=result.bean.total
    			}
    		},true);
			var pageUrl;
			var routedata = {
			...
			Util.ajax.postJson(
				...
				_options.srInfo = json2.beans[0];
				pageUrl = json2.beans[0].reqstUrlAddr; //REQST_URL_ADDR
		    })
			var config = {
				el:$('#aor_tabContainer'),
				...
				tabs:[
				      {
				    	  ...
				    	  click:function(e, tabData){
				    		  require([''+pageUrl],function(BasicMessage){
			                      ...
			                      setInputsVal();
			                      if(workItem== null || workItem.nodeTypeCd!="01"){
			                    	// 在工单详情页面的输入框是不可编辑的状态
				    		  ...
				    		  $("div.operateBtn").removeClass("hide").addClass('show');
				    		  $("#faf_impList").removeClass("hide").addClass('show');
				    	  }
				      },
				      {
				    	  title:'流程记录',
...
//		    	//登录人员id
//		    	_options.loginStaffId = result.bean.staffId;
//            	//登录人员名称
//		    	_options.loginStaffName = result.bean.staffName;
//		    });
			fafList({}); //初始化列表
		    eventInit();
		    editStyle();
